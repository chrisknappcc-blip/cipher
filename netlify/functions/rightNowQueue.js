...
function scoreSignalForQueue(signal) {
  ...
  if (replied) {
    ...
    whyTag = hoursAgo != null && hoursAgo >= 1
      ...
      : `Just replied`;
  } else if (eventType === "SENT_NO_REPLY") {
    // We sent something (often via Outlook, outside HubSpot's own tracked
    // send) and haven't heard back — ages up the same way an unanswered
    // reply does, but the label is honest about direction: we reached out,
    // they haven't responded, not the other way around.
    const mult = agingMultiplier("reply_unanswered", hoursAgo);
    queueScore = queueScore * mult;
    whyTag = hoursAgo != null && hoursAgo >= 1
      ? `Sent ${Math.round(hoursAgo)}h ago, no response yet`
      : `Just sent`;
  } else {
